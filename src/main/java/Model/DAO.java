...
//		}
	...
	public int updateCitizen(String user, String pass) {
		int i = 0;
		Connection connection = ConnectionJDBC.getJDBC();
		try {
			String query = "UPDATE `citizen`.`account` SET `password` = ? "
							+ "WHERE (`username` = ?);";
			ps = connection.prepareStatement(query);
			ps.setString(2, user);
			ps.setString(1, pass);
	...
	public List<Citizen> getAllCitizenByLikeVillageID(String villageID, String value, String type) {
		villageID += "%";
		List<Citizen> list = new ArrayList<>();
		String sort = (value.equals("default")) ? "" : (value.equals("bot") ? "order by "+ type +" desc" : "order by "+ type +" asc");
		Connection connection = ConnectionJDBC.getJDBC();
		try {
			String query = "SELECT * FROM citizen.infomation where villageID like ? " + sort;
			ps = connection.prepareStatement(query);
			ps.setString(1, villageID);
	...
	public int updateCadresAccessTime(String username, String accesstime) {
		int i=0;
		Connection connection = ConnectionJDBC.getJDBC();
		try {
			String query = "UPDATE `citizen`.`account` SET `createTime` = ? WHERE (`username` = ?);";
			ps = connection.prepareStatement(query);
	...
	public static void main(String[] args){
		...
		for(Cadres cadres : list) {
			String date = "2021-01-12" + " 00:00:00";
			cadres.setCreateTime(date);
			count += dao.updateCadresAccessTime(cadres.getUsername(), cadres.getCreateTime());
